...
#define INA3221_POWER_UPPER        0x10
#define INA3221_POWER_LOWER        0x11
...
#define INA3221_DEFAULT_POWER_UPPER        (10000)
#define INA3221_DEFAULT_POWER_LOWER        (9000)
 ...
 * @pv_max: power-valid upper-limit
 * @pv_min: power-valid lower-limit
 */
struct ina3221_data {
...
static int ina3221_reg_transfer(u8 reg, long *val, unsigned int *regval, bool val_to_reg)
{
	switch (reg) {
	case INA3221_POWER_UPPER:
	case INA3221_POWER_LOWER:
		/* LSB=8 mV, last 3 bits unused */
        if (val_to_reg) {
            *regval = (DIV_ROUND_CLOSEST(clamp_val(*val, 0, 32760), 8) << 3);
        } else {
            *val = (sign_extend32(*regval >> 3, 12)) * 8;
        }
        break;
    default:
        return -EINVAL;
...
static int ina3221_reg_rw(struct ina3221_data *ina, u8 reg, long *transfer_val, unsigned int *regval, bool is_rd)
{
    int ret;
    ...
    if (is_rd) {
        ret = regmap_read(ina->regmap, reg, regval);
        if (ret < 0) {
            dev_err(ina->pm_dev, "regmap read reg:0x%x fail, ret: %d\n", reg, ret);
            return ret;
        }
        dev_dbg(ina->pm_dev, "regmap read reg:0x%x success, regval: 0x%x\n", reg, *regval);

        ret = ina3221_reg_transfer(reg, transfer_val, regval, false);
        if (ret < 0) {
            dev_err(ina->pm_dev, "ina3221_reg_transfer fail reg:0x%x, regval:0x%x, ret: %d\n", reg, *regval, ret);
            return ret;
        }
        dev_dbg(ina->pm_dev, "ina3221_reg_transfer success reg:0x%x, regval:0x%x, transfer_val: %ld\n", reg, *regval, *transfer_val);
    } else {
        ret = ina3221_reg_transfer(reg, transfer_val, regval, true);
        if (ret < 0) {
            dev_err(ina->pm_dev, "ina3221_reg_transfer fail reg:0x%x, regval:0x%x, ret: %d\n", reg, *regval, ret);
            return ret;
        }
        ...
        if (ret < 0) {
            dev_err(ina->pm_dev, "regmap write reg:0x%x fail, ret: %d\n", reg, ret);
            return ret;
        }
        dev_dbg(ina->pm_dev, "regmap write reg:0x%x success, regval: 0x%x\n", reg, *regval);
    }

    return 0;
}

static ssize_t ina3221_value_store(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret < 0) {
        dev_err(ina->pm_dev, "ina3221_value_store err, reg:0x%x, ret:%d\n", reg, ret);
		return ret;
    }

    mutex_lock(&ina->lock);
    ret = ina3221_reg_rw(ina, reg, &val, &regval, false);
    mutex_unlock(&ina->lock);
    if (ret < 0) {
        dev_err(ina->pm_dev, "ina3221_value_store fail, reg:0x%x, val:%ld, ret:%d\n", reg, val, ret);
        return ret;
    }
    dev_dbg(ina->pm_dev, "ina3221_value_store success reg:0x%x, regval:0x%x, transfer_val: %ld\n", reg, regval, val);

	return count;
}

static ssize_t ina3221_value_show(struct device *dev,
				 struct device_attribute *da, char *buf)
{
    ...
    mutex_lock(&ina->lock);
    ret = ina3221_reg_rw(ina, reg, &val, &regval, true);
    mutex_unlock(&ina->lock);
    if (ret < 0) {
        dev_err(ina->pm_dev, "ina3221_value_show fail, reg:0x%x, ret:%d\n", reg, ret);
        return ret;
    }
    dev_dbg(ina->pm_dev, "ina3221_value_show success reg:0x%x, regval:0x%x, transfer_val: %ld\n", reg, regval, val);
...
static SENSOR_DEVICE_ATTR_RW(pv_max, ina3221_value, INA3221_POWER_UPPER);
static SENSOR_DEVICE_ATTR_RW(pv_min, ina3221_value, INA3221_POWER_LOWER);

static struct attribute *ina3221_attrs[] = {
    &sensor_dev_attr_shunt1_resistor.dev_attr.attr,
    &sensor_dev_attr_shunt2_resistor.dev_attr.attr,
    &sensor_dev_attr_shunt3_resistor.dev_attr.attr,
    &sensor_dev_attr_pv_max.dev_attr.attr,
    &sensor_dev_attr_pv_min.dev_attr.attr,
    NULL,
};
...
static const struct regmap_range ina3221_yes_ranges[] = {
    ...
    regmap_reg_range(INA3221_POWER_UPPER, INA3221_POWER_LOWER),
};
...
static int ina3221_set_pv_threshold(struct ina3221_data *ina) {
    int ret;
    unsigned int regval;
...
static int ina3221_probe(struct i2c_client *client)
{
    ...
    ret = ina3221_set_pv_threshold(ina);
    if (ret < 0) {
        dev_err(dev, "Set pv threshold fail, ret: %d\n", ret);
        goto fail;
    }
