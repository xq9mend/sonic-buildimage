...
def g_run_command(table, command, use_bgpd_client, daemons, ignore_fail = False):
    ...
    if isinstance(command, list):
        is_vtysh = (len(command) > 0 and command[0] == 'vtysh')
    else:
        is_vtysh = command.startswith('vtysh ')
    if not is_vtysh:
    ...
    if use_bgpd_client:
        ...
        if isinstance(command, list):
            ...
            # Fallback for string commands: use shlex to avoid shell injection.
            # ValueError on malformed strings (unmatched quotes) is caught and rejected.
            import shlex
            try:
                cmd_args = shlex.split(command)
            except ValueError:
                syslog.syslog(syslog.LOG_ERR, '[bgp cfgd] invalid command string rejected: "{}"'.format(command))
                return False
